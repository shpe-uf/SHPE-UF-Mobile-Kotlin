package com.example.shpe_uf_mobile_kotlin.ui.theme

import androidx.compose.ui.graphics.Color

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)

val GBMColor = Color(0xFF3A3A3A)
val GBMColorLight = Color(0xFF4A4A4A)

val InfoSessionColor = Color(0xFF42757C)
val InfoSessionColorLight = Color(0xFF4A7C7C)

val WorkshopColor = Color(0xFFB08900)
val WorkshopColorLight = Color(0xFFB09C00)

val SocialColor = Color(0xFFA20000)
val SocialColorLight = Color(0xFFA21A00)

val VolunteeringColor = Color(0xFF2A5616)
val VolunteeringColorLight = Color(0xFF2A6A16)

val TextColor = Color(0xFFB7B7B7)

// Notification Colors
val allNotificationsOn = Color(0xFFD25917)
val allNotificationsOff = Color(0xFF933815)

